...
def test_aim_function_deriv_strains_one_stress_entry(plot=False):
    """ Check the implementation of the partial derivative of the aim
        function with respect to the strains for two load cases and
        considering only the yy-entry of the stress tensor.
    """

    ### ----- Set-up ----- ###
    # Consider only the yy-direction of the stress tensor
    case_stress_entry = 2
    ...
    for DelF in DelFs:
        ...
        stress = stress / loading / Young0
        target_stresses.append(stress)
    args = (target_stresses, weight, eta, loading, Young0, case_stress_entry)
    ...
    if plot:
        ...
        fig, ax = plt.subplots()
        ax.set_xlabel('Fin. diff.')
        ax.set_ylabel('Abs error of partial deriv aim_function w.r.t. strains (only yy-stress)')
        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.plot(delta_list, diff_list, marker='o', label='Calculated')
        delta_list = np.array(delta_list)
