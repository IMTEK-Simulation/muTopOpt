...
def test_square_error_target_stresses_deriv_strain_one_stress_entry(plot=False):
    """ Check the implementation of the derivative of the target stresses
        term with respect to the strains for two load cases and considering
        only one entry of the stress tensor.
    """

    ### ----- Set-up ----- ###
    # Consider only the xx-direction of the stress tensor
    case_stress_entry = 1
    ...
    # Function and derivative
    sq_err = square_error_target_stresses(cell, strains, stresses, target_stresses,
                                          loading, Young0, case_stress_entry)
    derivs = square_error_target_stresses_deriv_strains(cell, strains, stresses,
                                                        target_stresses, loading,
                                                        Young0, case_stress_entry)
    ...
    if plot:
        ...
        fig, ax = plt.subplots()
        ax.set_xlabel('Fin. diff.')
        ax.set_ylabel('Error of partial deriv StressTarget w.r.t. strains (only xx-stress)')
        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.plot(delta_list, diff_list, marker='o', label='Calculated')
        delta_list = np.array(delta_list)
...
def test_square_error_target_stresses_deriv_phase_one_stress_entry(plot=False):
    """ Check the implementation of the derivative of the target stresses
        term with respect to the phase for two load cases and considering only
        the xx-entry of the stress tensor.
    """
    ...
    # Function and derivative
    sq_err = square_error_target_stresses(cell, strains, stresses, target_stresses,
                                          loading, delta_Young, case_stress_entry)
    deriv = square_error_target_stresses_deriv_phase(cell, stresses, target_stresses,
                                                     dstress_dphase_list, loading,
                                                     delta_Young, case_stress_entry)
    ...
    if plot:
        ...
        fig, ax = plt.subplots()
        ax.set_xlabel('Fin. diff.')
        ax.set_ylabel('Error of partial deriv StressTarget w.r.t. phase (only xx-stress)')
        ax.set_xscale('log')
        ax.set_yscale('log')
        ax.plot(delta_list, diff_list, marker='o', label='Calculated')
        delta_list = np.array(delta_list)
