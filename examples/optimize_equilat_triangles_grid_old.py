"""
@file   optimize_equilat_triangles_grid.py

@author Indre Jödicke <indre.joedicke@imtek.uni-freiburg.de>

@date   26 Jun 2023

@brief  Example to optimize for an isotrop material using a
...
dim = len(nb_grid_pts)
hx = length_0 / nb_grid_pts[0]
hy = np.sqrt(3) / 2 * hx

lengths = [length_0, hy * nb_grid_pts[1]]
...
# Use linear finite elements (equilateral triangles)
...
folder = 'results_equilat_triangles_grid'
...
args = (target_stresses, w, eta, loading, Young2) # Arguments passed to the aim function
...
if MPI.COMM_WORLD.rank == 0:
    ...
    print('  Grid: Equilateral triangles')
...
if MPI.COMM_WORLD.rank == 0:
    with open(file_name, 'w') as f:
        ...
        print('nb_grid_pts, lenghts, weight, '
              ...
              'gtol, ftol, '
              'maxcor, applied_strain, target_shear_modulus, target_lame2, '
        ...
        np.savetxt(f, [DelFs[0][0, 0]], delimiter=' ', newline=' ')
...
# Initial phase distribution: sinus
x = np.linspace(0, lengths[0], nb_grid_pts[0], endpoint=False)
x = x + hx/2
y = np.linspace(0, lengths[1], nb_grid_pts[1], endpoint=False)
y = y + hy/2
phase_ini = np.empty(nb_grid_pts)
for j in range(nb_grid_pts[1]):
    phase_ini[:, j] = 0.25*np.sin(2*np.pi/lengths[0]*x) + 0.25
for i in range(nb_grid_pts[0]):
    phase_ini[i, :] += 0.25*np.sin(2*np.pi/lengths[1]*y) + 0.25
...
def fun(x):
    return call_function_parall(x, *opt_args)
